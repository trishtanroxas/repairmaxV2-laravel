    ...
    <div class="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                <span class="material-symbols-outlined text-[36px] text-blue-600">mail</span>
                Notification Inbox
                @if($unreadCount > 0)
            ...
            <p class="text-gray-500 mt-1">Review system updates, appointments status change, and user inquiries in a unified inbox.</p>
        </div>
                        ...
                        <div class="border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                            ...
                            @if(strtolower($selectedNotification->related_model) === 'appointment')
                                ...
                                <div class="p-5 md:p-6 bg-white space-y-5">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Customer Name</span>
                                            <span class="font-bold text-gray-900 mt-1 block">{{ $relatedDetails->user?->getFullName() ?? 'Guest Client' }}</span>
                                        </div>
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Contact Email</span>
                                            <span class="font-bold text-gray-900 mt-1 block font-mono">{{ $relatedDetails->user?->email ?? 'N/A' }}</span>
                                        </div>
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Contact Phone</span>
                                            <span class="font-bold text-gray-900 mt-1 block">{{ $relatedDetails->user?->phone ?? 'N/A' }}</span>
                                        </div>
                                        <div>
                                    ...
                                    <div class="pt-4 border-t border-gray-100 flex justify-end">
                                        <a href="{{ route('admin.appointment.details', $relatedDetails->id) }}" 
                            ...
                            @elseif(strtolower($selectedNotification->related_model) === 'message')
                                ...
                                <div class="p-5 md:p-6 bg-white space-y-5">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Sender Name</span>
                                            <span class="font-bold text-gray-900 mt-1 block">{{ $relatedDetails->user?->getFullName() ?? 'Guest User' }}</span>
                                        </div>
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Sender Email</span>
                                            <span class="font-bold text-gray-900 mt-1 block font-mono">{{ $relatedDetails->user?->email ?? 'N/A' }}</span>
                                        </div>
                            ...
                            <!-- User Accounts Entity Details -->
                            @elseif(strtolower($selectedNotification->related_model) === 'user')
                                <div class="px-5 py-4 bg-purple-600 text-white flex justify-between items-center">
                                    <h3 class="font-bold flex items-center gap-2 text-sm text-white">
                                        <span class="material-symbols-outlined text-white">person</span>
                                        User Directory Entry
                                    </h3>
                                    <span class="text-xs bg-white/20 px-3 py-1 rounded-lg font-bold border border-white/15 text-white">
                                        UID #{{ $relatedDetails->id }}
                                    </span>
                                </div>
                                <div class="p-5 md:p-6 bg-white space-y-5">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Full Name</span>
                                            <span class="font-bold text-gray-900 mt-1 block">{{ $relatedDetails->getFullName() }}</span>
                                        </div>
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Email Address</span>
                                            <span class="font-bold text-gray-900 mt-1 block font-mono">{{ $relatedDetails->email }}</span>
                                        </div>
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">Contact Phone</span>
                                            <span class="font-bold text-gray-900 mt-1 block">{{ $relatedDetails->phone ?? 'Not Provided' }}</span>
                                        </div>
                                        <div>
                                            <span class="text-[10px] uppercase font-mono font-bold text-gray-400 block tracking-wider">System Role</span>
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-purple-100 text-purple-800 capitalize mt-1">
                                                {{ $relatedDetails->role }}
                                            </span>
                                        </div>
                                    </div>
                        ...
                        @if($selectedNotification->related_model)
                            <div class="border border-dashed border-gray-200 bg-gray-50/50 rounded-2xl p-6 text-center text-gray-500">
                                ...
                                <p class="text-xs text-gray-500 mt-1">
                                    The related {{ strtolower(str_replace('_', ' ', $selectedNotification->related_model)) }} record may have been deleted, archived, or is inaccessible.
                                </p>
                            </div>
                        @endif
                    @endif
            ...
            @else
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-8 text-center text-gray-500">
                    ...
                    <p class="text-sm text-gray-500 mt-1 max-w-sm mx-auto">
                        Click on any notification in the inbox directory list to review its complete details and launch associated sheets.
                    </p>
                </div>
            @endif
        </div>
